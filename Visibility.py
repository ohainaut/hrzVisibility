...
#
# from script:
# plotIt(ephall, cystart,cyend, Deltamax=Deltamax)
#  
...
#------------------------------------------------------------------------------
def make_epochs(Ts, Te):
    '''generate epochs json for Hrz'''

    if (Te-Ts).value > 1000.:
        return {'start': Ts.value,
                  'stop' : Te.value,
                  'step' : '10d' }
    else:
        return {'start': Ts.value,
                  ...
                  'step' : '1d' }
#------------------------------------------------------------------------------
def computeObs(ephall, LatOBSr, blackRA, blackDec):
    '''compute the object visibility

    IN:
    - ephall: Table with the ephemerides
    - LatObsr: latitude [rad] of the observatory
    - blackRA, blackDec: position of the antiSun; same epochs as ephAll
    ...
    DECr = np.array(np.radians( ephall['DEC']))
...
#------------------------------------------------------------------------------
def doXticks(ax, tickMonthFlag, tickmonths, tickyears, limityears):
    # deal with the X-ticks for the plots
    if tickMonthFlag:
        ax.set_xticks(tickmonths.jd)
    else:
        ax.set_xticks(tickyears.jd)
        ax.set_xticks(tickmonths.jd, minor=True)
    ax.set_xticklabels([])
    ax.grid(axis='x',color='k',linewidth=0.1,alpha=0.1,linestyle='-', visible=True)
    ax.set_xlim(limityears.jd)

#------------------------------------------------------------------------------


def plotIt(ephall, cystart,cyend, Deltamax=0, absMag=15.):
    '''Generate the visibility plot
    
    in: 
    - ephall: a Table with the ephemerides of the object
    - cystart, end: start and end of the plot, in YYYY-MM-DD

    out: generates the plt (but does not save it)
    ...
    w = (ephall["datetime_jd"]-JD0)/356.25*360.
    lsunr = Angle([x%360*u.deg for x in w])
    ...
    elif 'Nmag' in ephall.columns and ephall['Nmag'][0] != "--":
        ephall['magPh'] = ephall['Nmag']
    else:
        ephall['magPh'] = absMag + 5.*np.log10( ephall['delta'] *  ephall['r'] )
    ...
    tickMonthFlag = (limityears[1].jd - limityears[0].jd) < 700.
    ...
    ax1.set_ylabel(u"r (black)\n$\Delta$ (blue) [AU]",color=mycol)
    ax1.tick_params('y', colors=mycol)
    ax1.set_ylim(ymin,ymax)
    ax1.set_xlim(limityears.jd)
    ...
    doXticks(ax2, tickMonthFlag, tickmonths, tickyears, limityears)
    ...
    obsPAO, TPAO, nightLenPAO = computeObs(ephall, LatOBSr, blackRA, blackDec)
    ...
    obsLSO, TLSO, nightLenLSO = computeObs(ephall, LatOBSr, blackRA, blackDec)
    ...
    obsCAN, TCAN, nightLenCAN = computeObs(ephall, LatOBSr, blackRA, blackDec)
    ...
    obsMKO, TMKO, nightLenMKO = computeObs(ephall, LatOBSr, blackRA, blackDec)
...
if __name__ == "__main__":
    ...
    Ts = Time(cystart, format='iso')
    Te = Time(cyend,   format='iso')
    epochs = make_epochs(Ts, Te)
